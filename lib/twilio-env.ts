...
export function saveTwilioEnv(env: TwilioEnv): { envPath: string; jsonPath: string } {
  ...
  const envContent = [
    "# GrayArx Twilio credentials",
    `TWILIO_ACCOUNT_SID=${env.accountSid}`,
    `TWILIO_AUTH_TOKEN=${env.authToken}`,
    `TWILIO_WEBHOOK_BASE_URL=${env.webhookBaseUrl}`,
    env.fromNumber ? `TWILIO_FROM_NUMBER=${env.fromNumber}` : "",
  ]
    .filter(Boolean)
    .join("\n");

  writeFileSync(ENV_PATH, `${envContent}\n`, "utf8");
  writeFileSync(JSON_PATH, `${JSON.stringify(env, null, 2)}\n`, "utf8");
  applyTwilioEnv(env);

  return { envPath: ENV_PATH, jsonPath: JSON_PATH };
}
