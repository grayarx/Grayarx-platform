...
import { buildMondayRoiReport, type RoiReport } from "@/lib/conversion/roi";
import { listPartsEnquiries } from "@/lib/os/parts";
...
export function buildMondayEmailBody(report: RoiReport): string {
  const parts = listPartsEnquiries().length;
...
export async function sendMondayRoiEmail(input: {
  ...
  const body = buildMondayEmailBody(report);
