...
| `POST /api/scheduled/prospect-enrich-tick` | optional external cron (e.g. hourly) | Sipho deep-fetches dealer websites and stores **named/principal** emails only. **Also always-on in-process:** digs **1 dealership every ~15 min** on a boot timer (no cron required). |
