...
- LinkedIn shows **people**, not inboxes. Sipho discovers principal *names* (site team pages + web search), then finds emails via **Hunter.io** (`HUNTER_API_KEY` on Railway — recommended) or SMTP verify when port 25 is open. See `principalNameEmailGuess.ts`.
